...
def main():
    ...
    # Load dataset
    dataset_name = "apartment"
    path = "data/apartment/apartments_for_rent_classified_100K.csv"
    data = pd.read_csv(path, sep=";", encoding='cp1252', on_bad_lines="skip", low_memory=False)

    # Delete some columns
    data = data.drop(["pets_allowed", "amenities", "address", "title", "body",
                      "currency", "price_display", "id", "cityname"], axis=1)

    # Some preprocessing
    data["category"] = data["category"].apply(lambda x:x.split('/')[-1] if x is not np.nan else "other")
    ...
    # Column to fit
    cols = data.columns.difference(["category", "cityname"]).tolist()

    # Model
    km = KMR(n_clusters=6, n_init=1, n_iter=25, seed=SEED);
    by = KMR(n_clusters=6, n_init=1, n_iter=25, seed=SEED, n_byzantines=n_byzantines);
    co = KMR(n_clusters=6, n_init=1, n_iter=25, seed=SEED, n_byzantines=n_byzantines, correction=True);
