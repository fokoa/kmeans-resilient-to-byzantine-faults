...
from functions import save_indices_score
...
def comparaison(X, label_km, label_by, label_co, name_data, n_workers, n_byz):
...
def main():
    ...
    dataset_name = "iris"
    X = iris[['a', 'b', 'c', 'd']].values;
    km = KMR(n_clusters=3, n_init=1, n_iter=25, seed=SEED)
    by = KMR(n_clusters=3, n_init=1, n_iter=25, seed=SEED, n_byzantines=n_byzantines)
    co = KMR(n_clusters=3, n_init=1, n_iter=25, seed=SEED, n_byzantines=n_byzantines, correction=True)
    ...
    # Plot clusters, inertia and indices
    if rank == 0:
